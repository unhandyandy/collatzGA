...
function policyIter(numiters::Int64,numeps::Int64,numwus::Int64,policy)
    ...
    for i = 1:numiters
        ...
        d = trainNet(examples,newpolicy)
        if d < 0.0001 break end
    end
    return newpolicy
end
...
function get_wts(policy::MCTSPlanner,state::TTTState)::Array{Float64,1}
    node = get_state_node(policy.tree.value,state)
    sanodes = collect(children(node))
    mvs = [ action(n) for n in sanodes ]
    function getN(m::TTTMove)::Int64
        if m in mvs
            ind = findfirst(n -> action(n)==m,sanodes)
            return MCTS.n(sanodes[ind])
        else
            return 0
        end
    end
...
function execEp(policy::MCTSPlanner,numwus::Int,depth::Int64=100,start::TTTState=startpos)::Array{Any,1}
    ...
    while true
        #println(numwus)
        for w = 1:numwus
            MCTS.simulate(policy,get_state_node(policy.tree.value,s,policy),9)
        end
...
#     sN = snode.N
#     ind = 0
#     pis = 0.9 * predict(state)[1] + 0.1 * dir_noise2(9)
...
function best_sanode_UCB(snode::StateNode, c::Float64)
    ...
    sn = total_n(snode)
    pis = 0.9 * predict(state(snode))[1] + 0.1 * dir_noise2(9)
    for sanode in children(snode)
        # if c == 0 || sn == 0 || (sn == 1 && n(sanode) == 0)
        #     UCB = q(sanode)
        # else
            #UCB = q(sanode) + c*sqrt(log(sn)/n(sanode))
        ind = POMDPs.action_index(policy.mdp,POMDPs.action(sanode))
        UCB = q(sanode) + c * pis[ind] * sqrt(n(sanode))/(sn + 1)
        # end
        # if isnan(UCB)
        #     @show sn
        #     @show n(sanode)
...
function trainNet(exs::AbstractArray{Any,1},policy)
    ...
    pdist = param_dist(old_params,new_params)
    println("prms d: ",pdist)
